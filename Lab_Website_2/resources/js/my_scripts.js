	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++)
			{//Outer for loop iterates over each row
				for(col_counter = 0; col_counter < table.rows[row_counter].cells.length; col_counter++)
				{
					if(col_counter == 1)
					{
						opp_team = table.rows[row_counter].cells[col_counter].innerHTML;//Read in a cells current value
					}
					if(col_counter == 2)
					{
						home = table.rows[row_counter].cells[col_counter].innerHTML;//Read in a cells current value
					}
					if(col_counter == 3)
					{
						opp = table.rows[row_counter].cells[col_counter].innerHTML;//Read in a cells current value
					}
					...
					{
						table.rows[row_counter].cells[4].innerHTML = opp_team;//Update the actual html of the cell
						score = false;
					}
...
function loadPlayersPage()
{
	...
	for(let player_index = 0; player_index < players.length; player_index++)
	{
		...
		aTag.innerText = players[player_index].name;
		mydiv.appendChild(aTag);
		aTag.onclick = function(){switchPlayers(player_index)};
	}
...
function switchPlayers(playerNum)
{
	...
	var average_pass = players[playerNum].pass_yards / players[playerNum].games_played;
	var average_rush = players[playerNum].rushing_yards / players[playerNum].games_played;
	var average_rec = players[playerNum].receiving_yards / players[playerNum].games_played;


	document.getElementById("avg_p_yards").innerHTML = Math.round(average_pass);
	document.getElementById("avg_r_yards").innerHTML = Math.round(average_rush);
